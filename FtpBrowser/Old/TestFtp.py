...
class util:
 ...
 class db:
  ...
  def __init__(self,mode=None):
   ...
   except:f=open(self.home+'db.dir', 'w');f.write(home);self.dir=self.home
   f.close()
...
class interface:
 ...
 def set(self):
  if app.body.current()==0:
   self.db.mod('settings_dir', ui.query(u'Settings Directory', 'text', u(self.db.get('settings_dir'))))
  elif app.body.current()==1:
   self.db.mod('defaultdir',ui.query(u'Default Directory', 'text', u(self.db.get('defaultdir'))))
  elif app.body.current()==2:
   self.db.mod('account_dir',ui.query(u'Account Directory', 'text', u(self.db.get('account_dir'))))
...
class ftpclass:
 ...
 def exit(self, action):app.exit_key_handler=action
 def disp(self, cback):return ui.Listbox(self.interface, cback)
 def login(self,ftpMgr,opts=None):
   ...
   if zipfile.is_zipfile(file):
    if ui.query(u'Zipfile detected, extract contents to current directory?', 'query'):self.up_zip(file)
     ...
     try:
      ...
      self.ftp.storbinary
      ('STOR '+n[len(n)-1], f, self.db.get('blocksize', 'int'))
 ...
 def disconnect(self,silent=False):
  try:
   if not silent:ui.note(u'Disconnecting...', 'info', 1)
   self.ftp.quit()
